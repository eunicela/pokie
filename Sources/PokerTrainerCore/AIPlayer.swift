...
public struct ArchetypeAIPlayer: AIPlayer, Sendable {
    ...
    private func betSize(in context: AIDecisionContext) -> Int {
        let target = max(GameEngine.defaultBigBlind, Int(Double(max(context.pot, GameEngine.defaultBigBlind)) * config.preferredBetPotFraction))
    ...
    private func raiseTarget(in context: AIDecisionContext) -> Int {
        ...
        let pressureRaise = context.currentStreetBet + max(context.toCall, Int(Double(max(context.pot, GameEngine.defaultBigBlind)) * config.preferredBetPotFraction))
        return min(context.currentStreetBet + context.stack, max(minRaiseTo, pressureRaise))
    }
